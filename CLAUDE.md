...
**Los 53 que faltan (todos multi-step profundos, 3-7 ops):** la clase estructural identificada que queda es el **solape parcial** — un quad fresco mitad-sobre una cara vieja del mismo plano, mitad en el aire (Ctrl-stack apilado sobre el techo de un túnel excavado en pared inclinada, prism 147): la exclusión de paridad por cara entera es incorrecta para ambas mitades; el fix real es clipear los triángulos del quad por el contorno de la cara vieja antes de contar (groundwork: `prune_plane_debris` extraído con flag). Además: la familia −50 degenerada (Ctrl-flush a través del sólido entero = hueco→túnel, cube 190 — semántica a decidir) y secuencias planta/grupo largas. Criterio invariable: minimizar seed → regresión nombrada → fix de raíz, nunca parche.
