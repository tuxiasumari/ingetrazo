...
| **🟡 G2** Terreno 3D drapeado *(G2-mínimo = muestreo DEM hecho 2026-07-09; malla 3D + drape + render PENDIENTE)* | DEM → muestreo de alturas (mín.) → malla display-only + ortofoto encima (full) | `georef/dem.py` HECHO: `DEMSource` (preset **AWS Terrain Tiles** terrarium, libre sin API key; encoding terrarium/mapbox), `decode_elevation` (NumPy, `h=R·256+G+B/256−32768`), `DEMSampler` (QObject, bilinear `elevation_at(lat,lon)`/`elevation_at_local`, reusa `TileFetcher`+`TileCache`, señal `changed`). **Falta:** heightfield→`TerrainObject` con VBO, drape planar de textura, render pass de terreno | **NumPy** (1er uso, instalado 2.5.1), Qt net | 🟡 **G2-mín ✅**: muestreo verificado (Lima 138 m, Cusco 3335 m), 11 tests; **full G2 pendiente** (ver terreno 3D con satélite drapeado, toggle, fit, nota ~30 m) |
