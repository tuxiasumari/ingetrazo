...
> - **Fuente XYZ personalizada PERSISTE (2026-07-15, ajuste pre-release):** la URL pegada en "XYZ personalizado" y la fuente elegida se guardan en `QSettings` (`basemap/custom_url`, `basemap/source`) y se restauran al construir el panel (`BaseMapPanel._restore_saved_source`/`_save_source_pref`); un campo vacío nunca borra la URL guardada. 3 tests en `tests/test_basemap_prefs.py` (primer módulo de tests con `QApplication` widgets offscreen — de paso destrabó los 3 tests que antes se salteaban).
